...
import { Article, ArticleProps, convertDbDataToPeopleProps } from '../../components/post/Article';
...
export const PeopleArticlePage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (id) {
      fetchGetRequest('http://localhost:8000/api/peoples/public/' + id).then(res => {
        if (res) {
          setData(convertDbDataToPeopleProps(res));
        }
      });
    }
  }, []);
